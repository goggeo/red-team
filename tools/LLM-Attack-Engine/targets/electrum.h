...
class ElectrumAttack {
public:
    ElectrumAttack(GPUManager* gpuManager, MLPredictor* mlPredictor, RuleEngine* ruleEngine, 
...
private:
    ...
    std::string electrumNodeUrl;
    std::string rpcUser;
    std::string rpcPassword;

    CURL* curl;

    void connectToElectrumNode();
    void disconnectFromElectrumNode();

    bool checkSeedInElectrumNode(const std::string& seed);
